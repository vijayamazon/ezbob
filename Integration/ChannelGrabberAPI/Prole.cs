...
namespace Integration.ChannelGrabberAPI {
	...
	public abstract class Prole : ConfigurationRoot {
		...
		protected Prole(ILog log, Customer customer) {
			m_oLog = log;
		...
		private XmlDocument ExecuteRequest(string sResource = "", IJsonable oData = null) {
			...
			if (sResource == string.Empty)
				return this.ExecuteRequest(this.CreateRequest(sResource));

			return this.ExecuteRequest(this.CreateRequest(sResource, oData));
		} // ExecuteRequest
		...
		public string ShopTypeName { get { return this.ShopType.ToString(); } }
		...
		protected void Info(string sMsg) { this.Say(LogMsgTypes.Info, sMsg); } // Info
		...
		protected void Error(string sMsg) { this.Say(LogMsgTypes.Error, sMsg); } // Error
		...
		protected void Warn(string sMsg) { this.Say(LogMsgTypes.Warn, sMsg); } // Warn
		...
		protected void Fatal(string sMsg) { this.Say(LogMsgTypes.Fatal, sMsg); } // Msg
		...
		protected void Debug(string sMsg) { this.Say(LogMsgTypes.Debug, sMsg); } // Debug
