namespace TeamCityModels.ActionTypes
{
    ...
    public class VcsRoots: IVcsRoots
    {
        ...
        {
            var vcsRootWrapper = this._caller.Get<VcsRootWrapper>("/app/rest/vcs-roots");
        ...
        {
            var vcsRoot = this._caller.GetFormat<VcsRoot>("/app/rest/vcs-roots/id:{0}", vcsRootId);
        ...
        public VcsRoot AttachVcsRoot(BuildTypeLocator locator, VcsRoot vcsRoot)
        {
            ...
            return this._caller.PostFormat<VcsRoot>(xml, HttpContentTypes.ApplicationXml, string.Empty, "/app/rest/buildTypes/{0}/vcs-root-entries", locator);	
        }

        public void DetachVcsRoot(BuildTypeLocator locator, string vcsRootId)
        {
            this._caller.DeleteFormat("/app/rest/buildTypes/{0}/vcs-root-entries/{1}", locator, vcsRootId);
        }

        public void SetVcsRootField(VcsRoot vcsRoot, VcsRootField field, object value)
        {
            this._caller.PutFormat(value, "/app/rest/vcs-roots/id:{0}/{1}", vcsRoot.Id, ToCamelCase(field.ToString()));
        }
