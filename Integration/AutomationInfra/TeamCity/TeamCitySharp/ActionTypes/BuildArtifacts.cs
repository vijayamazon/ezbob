namespace TeamCityModels.ActionTypes
{
    ...
    public class BuildArtifacts : IBuildArtifacts
    {
        ...
        {
            this._caller.GetDownloadFormat(downloadHandler, "/downloadArtifacts.html?buildId={0}", buildId);
        }

        public ArtifactWrapper ByBuildConfigId(string buildConfigId)
        {
            return new ArtifactWrapper(this._caller, buildConfigId);
        }
    }
    ...
    public class ArtifactWrapper
    {
        ...
        {
            this._caller = caller;
            this._buildConfigId = buildConfigId;
        }
        ...
        {
            var xml = this._caller.GetRaw(string.Format("/repository/download/{0}/{1}/teamcity-ivy.xml", this._buildConfigId, buildSpecification));
            ...
            foreach (XmlNode node in artifactNodes)
            {
                ...
                list.Add(string.Format("/repository/download/{0}/{1}/{2}", this._buildConfigId, buildSpecification, artifact));
            }
            return new ArtifactCollection(this._caller, list);
        }
    }
    ...
    public class ArtifactCollection
    {
        ...
        public List<string> Download(string directory = null, bool flatten = false, bool overwrite = true)
        {
            ...
            foreach (var url in this._urls)
            {
                ...
                this._caller.GetDownloadFormat(tempfile => File.Move(tempfile, destination), url);
            }
            return downloaded;
        }
    }
