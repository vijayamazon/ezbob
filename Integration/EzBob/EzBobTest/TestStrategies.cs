...
<AccountModel xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <publicCert>-----BEGIN CERTIFICATE----------END CERTIFICATE-----</publicCert>  <privateKey>-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----</privateKey>
		...
		public void TestAutoApprove() {
			int nAutoApproveMismatch = 0;
			int nOtherErrorCount = 0;

			List<AutoApproveInputRow> lst = m_oDB.Fill<AutoApproveInputRow>(@"
SELECT TOP 1000
	mc.CustomerId,
	mc.Medal,
...
WHERE
	mc.IsActive = 1", CommandSpecies.Text);

			for (int i = 0; i < lst.Count; i++) {
				m_oLog.Debug("Customer {0} out of {1}, id {2}...", i + 1, lst.Count, lst[i].CustomerId);

				var adr = TestOneAutoApprove(lst[i]);

				if (adr == null)
					nOtherErrorCount++;
				else if (adr.LoanOfferUnderwriterComment.StartsWith("Mismatch"))
					nAutoApproveMismatch++;

				m_oLog.Debug("Customer {0} out of {1}, id {2} complete, result: {3}.",
					i + 1, lst.Count, lst[i].CustomerId,
					adr == null ? "NULL" : adr.LoanOfferUnderwriterComment
				);
			} // for

			m_oLog.Debug("Auto approval: sent {0}, mismatch {1}, other errors {2}.", lst.Count, nAutoApproveMismatch, nOtherErrorCount);

			Assert.AreEqual(0, nAutoApproveMismatch + nOtherErrorCount);
		} // 

		private class AutoApproveInputRow {
			public int CustomerId { get; set; }
			public string Medal { get; set; }
			public int OfferedLoanAmount { get; set; }
		}

		private AutoDecisionResponse TestOneAutoApprove(AutoApproveInputRow r) {
			MedalClassification nMedal;

			if (!Enum.TryParse(r.Medal, out nMedal)) {
				m_oLog.Alert("Failed to parse medal '{0}' for customer {1}.", r.Medal, r.CustomerId);
				return null;
			} // if

			var dataGatherer = new DataGatherer(r.CustomerId, m_oDB, m_oLog);
			...
			new Approval(
				r.CustomerId,
				dataGatherer.BrokerId.HasValue,
