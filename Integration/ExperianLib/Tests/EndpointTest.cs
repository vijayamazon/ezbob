...
namespace ExperianLib.Tests
{
    internal class EndpointTest : BaseTest
    {
        ...
        {
            var service = new IdHubService();
            var results = service.AccountVerification("Andrew", "JB", "Milburn", "M", new DateTime(1964, 12, 8), null,
                                                      null, "3 Linford Close", "Rugeley", null, "WS15 4EF", "110730",
                                                      "00742205", 1);
            if (!results.HasError)
            {
                Assert.Pass("BWA test passed.");
            }
            else
            {
                Assert.Fail("BWA test fail: " + results.Error);
            }
        }
        ...
        public void TestAdamPhilpottBwa()
        {
            ...
            var results = service.AccountVerification("Adam", null, "Philpott", "M", new DateTime(1987, 12, 16),
                                                      "33 North Court Close", "Rustington", null, "Littlehampton", null,
                                                      "BN16 3HZ", "306782", "23356268", 1);
            if (!results.HasError)
            {
                Assert.Pass("BWA test passed.");
            }
        ...
        {
            var service = new IdHubService();
            var results = service.AccountVerificationForcedWithCustomAddress("Adam", null, "Philpott", "M",
                                                                             new DateTime(1987, 12, 16),
                                                                             "33 North Court Close", "Rustington", null,
                                                                             "Littlehampton", null, "BN16 3HZ", "306782",
                                                                             "23356268",
                                                                             "", 1, BwaXml);
            if (!results.HasError)
            {
        ...
        {
            var service = new IdHubService();
            var results = service.AuthenticateForcedWithCustomAddress("Adam", null, "Philpott", "M",
                                                                      new DateTime(1987, 12, 16),
                                                                      "33 North Court Close", "Rustington", null,
