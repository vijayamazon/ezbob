namespace PayPoint
{
	...
	public class PayPointConnector {
		...
		public static PayPointDataSet.TransactionDataTable GetOrders(string condition, string mid, string vpnPassword, string remotePassword) {
			try {
				return GetOrdersDragonStyle(condition, mid, vpnPassword, remotePassword);
			}
			catch (Exception e) {
				log.ErrorFormat("Error getting PayPoint orders. Marketplace will appear empty. Try to recheck later. Exception: {0}", e);
			} // try

			return new PayPointDataSet().Transaction;
		} // GetOrders

		public static PayPointDataSet.TransactionDataTable GetOrdersDragonStyle(string condition, string mid, string vpnPassword, string remotePassword) {
