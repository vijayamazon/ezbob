...
namespace Deveel.Web.Zoho {
	public sealed partial class ZohoCrmClient {
		...
		{
		    var response = PostDataInternal(module, method, parameters, xmlData);

		    return new ZohoInsertResponse(module, method, response.Content);
		}

	    private IRestResponse PostDataInternal(string module, string method, IDictionary<string, string> parameters, string xmlData)
	    {
	        if (module == null)
        ...
        public ZohoConvertLeadResponse ConvertLead(string leadId)
        {
            var potential = new ZohoPotential("");
            var collection = new ZohoEntityCollection<ZohoPotential> { potential };
            
            var p = new Dictionary<string, string> {{"leadId", leadId}, {"newFormat", "2"}, {"version", "2"}};

            potential.SetValue("createPotential", false);
            potential.SetValue("assignTo", "nimrodk@ezbob.com");
            potential.SetValue("notifyLeadOwner", true);
            potential.SetValue("notifyNewEntityOwner", true);

            var r = PostDataInternal("Leads", "convertLead", p, collection.ToXmlString());
