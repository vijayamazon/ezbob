...
update [dbo].[NL_PacnetTransactions] set 
	[TransactionTime]=DATEADD(MONTH, @monthBack, 
	[TransactionTime]),[StatusUpdatedTime]=DATEADD(MONTH, @monthBack, [StatusUpdatedTime]) where [FundTransferID]= (select FundTransferID from NL_FundTransfers where LoanID=@loanID);
