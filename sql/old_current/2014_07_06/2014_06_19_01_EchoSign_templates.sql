...
BEGIN
	CREATE TABLE EsignTemplateTypes (
		EsignTemplateTypeID INT NOT NULL,
		DocumentName NVARCHAR(256) NOT NULL,
		FileNameBase NVARCHAR(256) NOT NULL,
		TimestampCounter ROWVERSION,
		CONSTRAINT PK_EsignTemplateTypes PRIMARY KEY(EsignTemplateTypeID)
	)
END
GO

IF NOT EXISTS (SELECT * FROM EsignTemplateTypes WHERE EsignTemplateTypeID = 1)
	INSERT INTO EsignTemplateTypes (EsignTemplateTypeID, DocumentName, FileNameBase) VALUES (1, 'Board resolution', 'board_resolution')
GO

IF NOT EXISTS (SELECT * FROM EsignTemplateTypes WHERE EsignTemplateTypeID = 2)
	INSERT INTO EsignTemplateTypes (EsignTemplateTypeID, DocumentName, FileNameBase) VALUES (2, 'Personal guarantee', 'personal_guarantee')
GO

IF OBJECT_ID('EsignTemplates') IS NULL
BEGIN
	CREATE TABLE EsignTemplates (
		...
		TimestampCounter ROWVERSION,
		CONSTRAINT PK_EsignTemplates PRIMARY KEY(EsignTemplateID),
		CONSTRAINT FK_EsignTemplates_Type FOREIGN KEY (EsignTemplateTypeID) REFERENCES EsignTemplateTypes (EsignTemplateTypeID)
	)
END
GO

IF NOT EXISTS (SELECT * FROM EsignTemplates WHERE EsignTemplateID = 1)
BEGIN
	INSERT INTO EsignTemplates(
		EsignTemplateID, DateAdded, EsignTemplateTypeID, FileExtension,
		Template
	) VALUES (
		1, GETUTCDATE(), 1, 'html',
		CONVERT(VARBINARY(MAX), '<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
...
<h4 style="text-align: center;">OF __COMPANY_NAME__ (the “Company”)</h4>
...
<dl>
	...
	in connection with the Company’s business operations and activities;
...
IF NOT EXISTS (SELECT * FROM EsignTemplates WHERE EsignTemplateID = 2)
BEGIN
	INSERT INTO EsignTemplates(
		EsignTemplateID, DateAdded, EsignTemplateTypeID, FileExtension,
		Template
...
<p>In consideration of Orange Money Ltd. of 145 – 157, St John Street, London, EC1V 4PY,
COMPANY NUMBER 7852687, (“the Creditor”) entering into the Loan Agreement, and __GUARANTOR_DETAILS__
(the Guarantor) hereby unconditionally and irrevocably and
...
<dl>
	...
	to a loan in the original principal amount of __LOAN_AMOUNT__ (“Loan”).
...
<p>1.3.3 references to any person are to be construed to include that person’s assigns or transferees
...
demand, of the Principal Debtor’s Obligations.</p>

<p>3.2 If the Principal Debtor’s Obligations are not recoverable from the Principal Debtor by reason
...
shall notwithstanding any of the foregoing be liable under this Guarantee for the Principal Debtor’s
...
the Creditor under or in connection with the Principal Debtor’s Obligations.</p>
...
or against the Principal Debtor or any other person in respect of the Principal Debtor’s Obligations;</p>
...
Principle Obligor or any other person that the Loan Agreement and Principle Obligor’s
...
satisfaction of any of the Guarantor’s liabilities under clause 3 as the Creditor, in its absolute
...
of the Guarantor at any place or by despatching it addressed to the Guarantor at the Guarantor’s
