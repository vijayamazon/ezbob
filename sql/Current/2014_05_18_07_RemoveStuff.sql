...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetExceededApplication]') AND TYPE IN (N'P', N'PC'))
...
GO
IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetHistoryByApplicationId]') AND TYPE IN (N'P', N'PC'))
