...
;WITH EF AS (
	SELECT ef.CustomerMarketPlaceId, max(ef.Id) FeedbackId
	FROM MP_CustomerMarketPlace mp INNER JOIN MP_EbayFeedback ef ON mp.Id = ef.CustomerMarketPlaceId
	WHERE mp.CustomerId=@CustomerId AND mp.Disabled=0
	GROUP BY ef.CustomerMarketPlaceId
)
SELECT @EbayFeedbacks = isnull(sum(efi.Positive), 0) FROM MP_EbayFeedbackItem efi INNER JOIN EF ON efi.EbayFeedbackId = EF.FeedbackId
INNER JOIN MP_AnalysisFunctionTimePeriod tp ON tp.Id = efi.TimePeriodId
WHERE tp.InternalId = '16619B19-ABF5-4AE0-9040-93EFD2E71FDB' --0 all data

--num of paypal income transactions
SELECT @PaypalFeedbacks = count(*) 
FROM MP_CustomerMarketPlace mp INNER JOIN MP_PayPalTransaction ppt ON mp.Id=ppt.CustomerMarketPlaceId
INNER JOIN MP_PayPalTransactionItem2 ppti ON ppt.Id = ppti.TransactionId
WHERE mp.CustomerId=@CustomerId AND mp.Disabled=0 AND ppti.GrossAmount>0
