...
BEGIN
	...
	IF (@MPTypeId = @EbayId)
	BEGIN
		SELECT @MinAnnualizedRevenue = isnull(min(CAST(av.ValueFloat AS DECIMAL(18,4))),0) FROM MP_AnalyisisFunctionValues av INNER JOIN MP_AnalyisisFunction af ON af.Id = av.AnalyisisFunctionId
