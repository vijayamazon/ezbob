...
BEGIN
	...
	SELECT
		...
		CurrentMonth = 	dbo.udfMonthEnd(dbo.udfGetLatestTotalsMonth(h.CustomerMarketPlaceId, @Now))	-- with "month tail" consideration	
	FROM
		dbo.MP_CustomerMarketPlaceUpdatingHistory h
		INNER JOIN dbo.MP_CustomerMarketPlace m ON h.CustomerMarketPlaceId = m.Id AND m.CustomerId = @CustomerID
		INNER JOIN dbo.MP_MarketplaceType t ON m.MarketPlaceId = t.Id AND (ISNULL(@IsForApprove, 0) = 0 OR t.InternalId IN (@HMRC, @Yodlee, @eBay, @Amazon, @PayPal))
	WHERE
		h.UpdatingEnd <= @Now
		AND ISNULL(m.Disabled, 0) = 0		
		AND (LEN(h.Error) = 0 OR h.Error IS NULL) 
	GROUP BY
	...
	FROM
		#mp	INNER JOIN dbo.MP_CustomerMarketPlace m ON #mp.MpID = m.Id INNER JOIN dbo.MP_MarketplaceType mt ON m.MarketPlaceId = mt.Id
	...
	FROM
		...
		INNER JOIN #mp m ON t.CustomerMarketPlaceId = m.MpID AND t.TheMonth BETWEEN m.YearAgo AND m.CurrentMonth 
