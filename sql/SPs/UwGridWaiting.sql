...
IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[UwGridWaiting]') AND TYPE IN (N'P', N'PC'))
DROP PROCEDURE [dbo].[UwGridWaiting]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[UwGridWaiting] 
	(@WithTest BIT)
AS
BEGIN
	SELECT
...
WHERE
	...
	AND	c.CreditResult = 'WaitingForDecision'
ORDER BY
	c.Id DESC
END
GO
