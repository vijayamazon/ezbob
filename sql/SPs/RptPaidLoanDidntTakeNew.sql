IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RptPaidLoanDidntTakeNew]') AND TYPE IN (N'P', N'PC'))
DROP PROCEDURE [dbo].[RptPaidLoanDidntTakeNew]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[RptPaidLoanDidntTakeNew] 
	(@DateStart DATETIME,
				 @DateEnd DATETIME)
AS
BEGIN
