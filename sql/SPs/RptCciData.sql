IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[RptCciData]') AND TYPE IN (N'P', N'PC'))
DROP PROCEDURE [dbo].[RptCciData]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[RptCciData]
AS
BEGIN
	SET NOCOUNT ON
	...
	SELECT
		...
		CASE c.TypeOfBusiness
			WHEN 'Entrepreneur' THEN 'Personal'
			ELSE 'Business'
		END AS LoanType,
