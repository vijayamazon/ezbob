IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MP_FreeAgentOrderItem]') AND type in (N'U'))
...
CREATE TABLE [dbo].[MP_FreeAgentOrderItem](
 ...
 CONSTRAINT [PK_MP_FreeAgentOrderItem] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_MP_FreeAgentOrderItemOrderId] ON [dbo].[MP_FreeAgentOrderItem] 
(
	[OrderId] ASC
