...
BEGIN
	...
	DECLARE cur CURSOR FOR SELECT Id, Name FROM MP_MarketplaceType WHERE Name != 'PayPoint'
	OPEN cur
	FETCH NEXT FROM cur INTO @mpId, @mpName
	WHILE @@FETCH_STATUS = 0
	BEGIN
		SELECT Id INTO #NotFinishedCustomers FROM Customer WHERE GreetingMailSentDate >= @DateStart AND GreetingMailSentDate < @DateEnd AND WizardStep!=4 AND Customer.IsTest = 0
		SELECT Id, Gender, DATEDIFF(hour,DateOfBirth,GETDATE())/8766.0 AS Age INTO #FinishedCustomers FROM Customer WHERE Customer.IsTest = 0 AND GreetingMailSentDate >= @DateStart AND GreetingMailSentDate < @DateEnd AND WizardStep=4
	...
	BEGIN
		...
		BEGIN
			...
			WHILE @@FETCH_STATUS = 0
			BEGIN
				UPDATE #tmp SET NumOfShopsApproved = NumOfShopsApproved + 1 WHERE MarketPlaceId = @MarketPlaceTypeId
	...
	WHILE @@FETCH_STATUS = 0
	BEGIN
		UPDATE #tmp SET AvgAmountApproved = @ProratedLoan / NumOfShopsApproved WHERE MarketPlaceId = @MarketPlaceTypeId
		FETCH NEXT FROM cur INTO @MarketPlaceTypeId, @ProratedLoan
	END
	CLOSE cur
	DEALLOCATE cur
