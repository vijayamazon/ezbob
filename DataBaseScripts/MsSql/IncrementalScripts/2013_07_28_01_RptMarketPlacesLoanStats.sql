IF OBJECT_ID('RptMarketPlacesLoanStats') IS NOT NULL
	DROP PROCEDURE RptMarketPlacesLoanStats
GO

CREATE PROCEDURE RptMarketPlacesLoanStats
	@DateStart DATETIME,
	@DateEnd DATETIME
AS
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM ReportScheduler WHERE Type='RPT_MARKETPLACES_LOAN_STATS')
BEGIN
	INSERT INTO ReportScheduler	VALUES
	(
		'RPT_MARKETPLACES_LOAN_STATS'
		, 'MarketPlaces Loan Stats'
		, 'RptMarketPlacesLoanStats'
		, 0
		, 0
		, 0
		, 'Name, Approved, Loaned, Avg Turnover, Avg Approved, Avg Loan, Avg Score, % of Men, Avg Age, % Utilization (by num), % Utilization (by amount)'
		, 'Name, NumOfShopsApproved, NumOfShopsLoaned, AvgTurnover, AvgApprovedAmount, AvgLoanAmount, AvgScore, PercentMen, AvgAge, UtilizationByNum, UtilizationByAmount'
	...
	SELECT @reportId = Id FROM ReportScheduler WHERE Type = 'RPT_MARKETPLACES_LOAN_STATS'
