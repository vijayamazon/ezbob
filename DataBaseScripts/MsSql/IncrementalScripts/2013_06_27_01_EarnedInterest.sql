IF OBJECT_ID('RptEarnedInterest') IS NOT NULL
	DROP PROCEDURE RptEarnedInterest
GO
...
IF NOT EXISTS (SELECT * FROM ReportScheduler WHERE Type = 'RPT_EARNED_INTEREST')
BEGIN
	INSERT INTO ReportScheduler VALUES('RPT_EARNED_INTEREST', 'Earned Interest', 'RptEarnedInterest', 0, 0, 0,
		'Date,Client ID,Loan ID,Client Name,Client Email,Earned Interest,Loan Amount,Total Repaid,Principal Repaid,Level',
		'IssueDate,!ClientID,!LoanID,ClientName,ClientEmail,EarnedInterest,LoanAmount,TotalRepaid,PrincipalRepaid,{RowLevel',
		'nimrodk@ezbob.com,alexbo+rpt@ezbob.com', 1)
END
ELSE BEGIN
	UPDATE ReportScheduler SET
		Title = 'Earned Interest',
		StoredProcedure = 'RptEarnedInterest',
		IsDaily = 0,
		IsWeekly = 0,
		IsMonthly = 0,
		Header = 'Date,Client ID,Loan ID,Client Name,Client Email,Earned Interest,Loan Amount,Total Repaid,Principal Repaid,Level',
		Fields = 'IssueDate,!ClientID,!LoanID,ClientName,ClientEmail,EarnedInterest,LoanAmount,TotalRepaid,PrincipalRepaid,{RowLevel',
		IsMonthToDate = 1
	WHERE
		Type = 'RPT_EARNED_INTEREST'
END
GO
