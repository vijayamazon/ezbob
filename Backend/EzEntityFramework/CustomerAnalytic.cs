//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace EzEntityFramework
{
    using System;
    using System.Collections.Generic;
    
    public partial class CustomerAnalytic
    {
        public Nullable<int> CustomerID { get; set; }
        public Nullable<System.DateTime> AnalyticsDate { get; set; }
        public int PersonalScore { get; set; }
        public int PersonalMinScore { get; set; }
        public int PersonalMaxScore { get; set; }
        public int IndebtednessIndex { get; set; }
        public int ThinFile { get; set; }
        public int NumOfAccounts { get; set; }
        public int NumOfDefaults { get; set; }
        public int NumOfLastDefaults { get; set; }
        public int CompanyScore { get; set; }
        public decimal SuggestedAmount { get; set; }
        public decimal AnnualTurnover { get; set; }
        public Nullable<System.DateTime> IncorporationDate { get; set; }
    }
}
