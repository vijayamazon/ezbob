namespace Ezbob.Backend.Strategies.NewLoan {
	...
	public class RescheduleLoan<T> : AStrategy {
		...
		private void GetCurrentLoanBalance(bool inRescheduling = true) {
			...
			if (this.tLoan != null) {
				...
				this.Result.LoanCloseDate = this.tLoan.Schedule.OrderBy(s => s.Date)
					.Last()
					.Date;

				if (!this.ReschedulingArguments.RescheduleIn) {
					//  if Re date is later than loan "maturity" date - "IN" rescheduling can't be proceeded
					if (this.ReschedulingArguments.ReschedulingDate > this.Result.LoanCloseDate) {
			...
			if (this.tNLLoan != null) {
				...
				Log.Debug("NEW LOAN STRUCTURE NOT IMPLEMENTED YET (IN)");
			}
		}
		...
		private void RescheduleOutLoan() {

			if (this.ReschedulingArguments.RescheduleIn)
				return;

			// 3. intervals number - to be calculated

			List<ScheduledItemWithAmountDue> shedules = new List<ScheduledItemWithAmountDue>();
