	...
	public class BackfillAndRemoveLimitedMedal : AStrategy
	{
		...
		public override void Execute()
		{
			...
			foreach (var sr in readers)
			{
				...
				int numOfEntriesInMedalCalculationsTable = sr["NumOfEntriesInMedalCalculationsTable"];
				DateTime? calculationTime = sr["CalculationTime"];
				bool isLimited = typeOfBusiness == "LLP" || typeOfBusiness == "Limited";

				if (numOfEntriesInMedalCalculationsTable > 0 && (!isLimited || numOfHmrcMps > 1))
				{
					DB.ExecuteNonQuery("RemoveCustomerFromMedalCalculations", CommandSpecies.StoredProcedure, new QueryParameter("CustomerId", customerId));
				}
				else if (numOfEntriesInMedalCalculationsTable == 0 && isLimited && numOfHmrcMps < 2)
				{
