    ...
    public abstract class MedalCalculatorBase {
        ...
        protected virtual decimal CalculateTurnoverForMedal(DateTime calculationTime) {
            try
            {
                ...
                List<decimal> onlineResultList = new List<decimal>();

                onlineResultList.Add(list_t1.ElementAt(0) + Math.Max(list_t1.ElementAt(1), list_t1.ElementAt(2)));
                onlineResultList.Add(list_t3.ElementAt(0) + Math.Max(list_t3.ElementAt(1), list_t3.ElementAt(2)));
                onlineResultList.Add(list_t6.ElementAt(0) + Math.Max(list_t6.ElementAt(1), list_t6.ElementAt(2)));
                onlineResultList.Add(list_t12.ElementAt(0) + Math.Max(list_t12.ElementAt(1), list_t12.ElementAt(2)));

                // "and take the minimum among them"
                annualTurnover = (from decimal r in onlineResultList
                                  where r > 0
                                  select r).AsQueryable()
                    .Min();

                const double onlineMedalTurnoverCutoff = 0.7;

                if (hmrcAnnualTurnover > (decimal)onlineMedalTurnoverCutoff * annualTurnover)
                    return (hmrcAnnualTurnover < 0) ? 0 : hmrcAnnualTurnover;

                if (yoodleeAnnualTurnover > (decimal)onlineMedalTurnoverCutoff * annualTurnover)
                    return (yoodleeAnnualTurnover < 0) ? 0 : yoodleeAnnualTurnover;
