...
EzBob.BankAccountInfoView = Backbone.View.extend({
	...
	render: function () {
		...
		this.validator = EzBob.validateSortCode(this.form);

		this.$el.find("#SortCode #SortCode1").on("keyup change", function () {
			that.$el.find("#SortCode .SortCodeSplit").val(that.$el.find("#SortCode #SortCode1").val() + that.$el.find("#SortCode #SortCode2").val() + that.$el.find("#SortCode #SortCode3").val());
		});
		...
		$(".dashboard-steps > ul li[data-step-num=0]").addClass("completed complete").removeClass("active current");
		$(".dashboard-steps > ul li[data-step-num=1]").addClass("active current");
	...
	inputsChanged: function () {
		...
		self.$el.find('#AccountNumber').each(function() {
			var oThis = $(this);

			if (oThis.val())
				self.$el.find('#AccountNumberImage').field_status('set', (oThis.val().length == oThis.attr('maxlength')) ? 'ok' : 'fail');
			else
				self.$el.find('#AccountNumberImage').field_status('clear');
		});

		var nCount = 0, nGood = 0, nEmpty = 0;

		self.$el.find('.SortCode').each(function() {
			var oThis = $(this);

			nCount++;

			if (oThis.val()) {
				if (oThis.val().length == oThis.attr('maxlength'))
					nGood++;
			}
			else
				nEmpty++;
		...
		if ((nCount == 0) || (nCount == nEmpty))
			self.$el.find('#SortCodeImage').field_status('clear');
		else
			self.$el.find('#SortCodeImage').field_status('set', (nCount == nGood) ? 'ok' : 'fail');

		var button = self.$el.find('a.connect-bank');
