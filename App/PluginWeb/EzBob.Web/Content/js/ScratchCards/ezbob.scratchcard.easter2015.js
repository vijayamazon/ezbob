...
EzBob.ScratchCards.Easter2015 = EzBob.View.extend({
	initialize: function(options) {
		...
		$(window).on(this.resizeEventName, _.bind(this.positionEggs, this));
	}, // initialize
	...
	positionEggs: function() {
		var widthRatio = this.$playground.width() / this.nativeGeometries.bg.width;
		var heightRatio = this.$playground.height() / this.nativeGeometries.bg.height;

		var ratio = widthRatio < heightRatio ? widthRatio : heightRatio;

		var paddingX = (this.$playground.width() - this.nativeGeometries.bg.width * ratio) / 2.0;
		...
		for (var egg in this.nativeGeometries.eggs) {
			...
			this.$el.find('.egg-' + egg).css({
				width: parseInt(geometry.width * ratio) + 'px',
				height: parseInt(geometry.height * ratio) + 'px',
				top: parseInt(geometry.top * ratio) + 'px',
				left: parseInt(paddingX + geometry.left * ratio) + 'px',
			}).show();
		} // for
