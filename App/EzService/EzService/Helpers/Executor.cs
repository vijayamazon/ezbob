    ...
    internal class Executor {
        ...
        public ActionMetaData Execute(ExecuteArguments oArgs) {
            ...
            try {
                ...
                ConstructorInfo oCreator = oArgs.StrategyType.GetConstructors()
                    .FirstOrDefault(
                        ci => ci.GetParameters()
                            .Length == oArgs.StrategyArguments.Count
                    );
        ...
        private ActionMetaData ExecuteSync<T>(
            ...
            return ExecuteSync(out oInstance, new ExecuteArguments(args) {
                CustomerID = nCustomerID,
                UserID = nUserID,
            });
        } // ExecuteSync
        ...
        private ActionMetaData ExecuteSync<T>(out T oInstance, ExecuteArguments args) where T : AStrategy {
            ...
            try {
                ...
                ConstructorInfo oCreator = typeof(T).GetConstructors()
                    .FirstOrDefault(
                        ci => ci.GetParameters()
                            .Length == args.StrategyArguments.Count
                    );
