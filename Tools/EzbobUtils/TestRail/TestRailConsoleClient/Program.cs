namespace TestRailConsoleClient
{
    ...
    using TestRailConsoleClient.ExtensionMethods;
    ...
    class Program
    {
        ...
        public static TestRailClient GetTestRailClient()
        {
            ...
            return new TestRailClient(url, user, apikey);
        }

        public static void PrintCasesDependency()
        {
            var testRailClient = GetTestRailClient();
            var ezbobProject = testRailClient.Projects.FirstOrDefault(x => x.Name == "EZbob");
            var log = new StringBuilder();
            if (ezbobProject != null) {
                log.Append("Type,Suite Name,Case Title,Shared,Case	Depended,Case Reference\n");
                var suites = testRailClient.GetSuites(ezbobProject.ID);
                foreach (var suiteItem in suites)
                {
                    if (suiteItem.ID != null)
                    {
                        var cases = testRailClient.GetCases(ezbobProject.ID, suiteItem.ID.Value);
                        if (cases != null)
                        {
            ...
            System.IO.File.WriteAllText(@"E:\Dependency\log.csv", log.ToString());
        }
