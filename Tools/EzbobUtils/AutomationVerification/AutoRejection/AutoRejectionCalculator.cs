namespace AutomationCalculator
{
	...
	public class AutoRejectionCalculator
    {
		public bool IsAutoRejected(int customerId, out string reason)
		{
			...
			if (dbHelper.WasApprovedForLoan(customerId))
			{
			...
			if (experianScore < Constants.DefaultScoreBelow &&
			    dbHelper.HasDefaultAccounts(customerId, Constants.DefaultMinAmount, Constants.DefaultMinMonths))
			{
				reason = "Default";
				return true;
			}
